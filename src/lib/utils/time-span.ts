...
export class TimeSpan {
  ...
  public get hoursStr(): string {
    return this.hours.toString().padStart(2, "0");
  }

  public get minutesStr(): string {
    return this.minutes.toString().padStart(2, "0");
  }

  public get secondsStr(): string {
  ...
  public toNiceTimerString(): string {
    const hours = this.hours.toString();
    const minutes = this.minutes.toString();
    const seconds = this.seconds.toString();
    if (this.totalSeconds == 0) return '-';
    return `${this.hours > 0 ? hours + "h" : ""} ${this.minutes > 0 ? minutes + "m" : ""} ${this.seconds > 0 ? seconds + "s" : ""}`;
  }
}